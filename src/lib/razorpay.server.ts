...
export const createRazorpayOrder = createServerFn("POST", async (payload: { orderId: string }) => {
  ...
  try {
    ...
    if (!keyId || !keySecret) {
      console.warn("[Razorpay] Missing RAZORPAY_KEY_ID or RAZORPAY_KEY_SECRET. Falling back to mock Razorpay order.");
